...
fileInput("archivo_fechas", "Sube archivo de fechas clave (.txt)", accept = ".txt")
helpText("El archivo debe tener una sola columna, cada fila una fecha en formato YYYY-MM-DD.")
tableOutput("tabla_fechas_claves")
```

```{r}
output$tabla_fechas_claves <- renderTable({
  fc <- fechas_claves_visibles()
  
  if (length(fc) == 0) {
    tibble(Mensaje = "No hay fechas clave cargadas en el rango seleccionado")
  } else {
    tibble(`Fechas clave visibles` = format(fc, "%Y-%m-%d"))
  }
})
...
```{r}
# Vector por defecto (vacío)
fechas_claves_defecto <- as.Date(character())

# Lectura del archivo de fechas clave
# Lectura y conversión automática desde .txt
# Fechas clave desde .txt en formato texto YYYY-MM-DD
fechas_claves <- reactive({
  if (is.null(input$archivo_fechas)) return(as.Date(character()))
  raw <- readr::read_lines(input$archivo_fechas$datapath)
  # Forzar interpretación con formato explícito
  fechas <- as.Date(raw, format = "%Y-%m-%d")
  # Quitar posibles NAs por líneas vacías o mal formateadas
  fechas[!is.na(fechas)]
})



fechas_claves_visibles <- reactive({
  # req(fechas_claves())  # no usar aquí para permitir vacío
  fechas_claves()[fechas_claves() >= input$rango_fechas[1] & fechas_claves() <= input$rango_fechas[2]]
})
...
pct_dias <- reactive({
  ...
  round(dias_conectado / total_dias * 100, 1)
})


# % de clics totales del alumno
pct_clics <- reactive({
  dias_por_alumno <- metrics_global()$dias_por_alumno
  valor <- dias_por_alumno %>%
    filter(usuario == input$alumno) %>%
    pull(pct_clics)
  if (length(valor) == 0 || is.na(valor)) return(0)
  round(valor, 1)
})

# % promedio de clics/día
pct_clics_dia <- reactive({
  dias_por_alumno <- metrics_global()$dias_por_alumno
  valor <- dias_por_alumno %>%
    filter(usuario == input$alumno) %>%
...
indice_interaccion <- reactive({
  p1 <- pct_dias()
  p2 <- pct_clics()
  p3 <- pct_clics_dia()
  
  round((p1/100 * p2/100 * p3/100)^(1/3) * 100)
})
...
renderGauge({
  ...
  # Recalcular el denominador en este ámbito
  total_dias <- as.integer(input$rango_fechas[2] - input$rango_fechas[1] + 1)
  if (is.na(total_dias) || total_dias <= 0) total_dias <- 1

  # Tomar el valor desde el reactivo ya definido
  valor <- pct_dias()
  if (is.null(valor) || is.na(valor)) valor <- 0
  ...
  gauge(
    ...
    min = 0, max = 100,
    symbol = "%",
    label = paste("días posibles", total_dias),
    gaugeSectors(
      success = c(75, 100),
      warning = c(50, 75),
      danger  = c(0, 50)
...
renderGauge({
  ...
  max_val <- metrics_global()$dias_por_alumno %>%
    summarise(max_val = max(pct_clics, na.rm = TRUE)) %>%
    pull(max_val)
...
renderPlot({
  ...
  ggplot(clics_por_dia, aes(x = Fecha, y = n)) +
    ...
    geom_vline(xintercept = fechas_claves_visibles(), 
